...
struct AnsiTextLite
{
    ...
    {
        ///
        AnsiTextLite fg(AnsiColour colour) return { this.styleSet.fg = colour; this.styleSet.fg.isBg = IsBgColour.no; return this; } // @suppress(dscanner.style.long_line)
        ///
        ...
        ///
        AnsiTextLite bg(AnsiColour colour) return { this.styleSet.bg = colour; this.styleSet.bg.isBg = IsBgColour.yes; return this; } // @suppress(dscanner.style.long_line)
        ///
        ...
        auto toRange()
        {
            static struct Range
            {
                ...
                const(char)[] front() return
                {
                    switch(sliceCount)
                    {
                        case 0: return this.start[0..this.startSliceLength];
            ...
            Range r;
            r.startSliceLength = this.toFullStartSequence(r.start).length;
            r.middle = this.text;
                ...
                Switch: switch(commandAsNum)
                {
                    ...
                    case 48:
                        if(result == ReadResult.foundEndMarker)
                            return "Incomplete 'set foreground/background' command, expected another parameter, got none."; // @suppress(dscanner.style.long_line)
...
unittest
{
    ...
    assert("".ansiAsSections.empty);

    auto r = "No Ansi".ansiAsSections;
    assert(!r.empty);
    assert(!r.front.isAnsiSequence);
    assert(r.front.slice == "No Ansi");

    r = "\033[m".ansiAsSections;
    assert(!r.empty);
    assert(r.front.isAnsiSequence);
    assert(r.front.slice == "\033[m");

    r = "\033[38;2;255;128;64;1;4;48;5;2m".ansiAsSections;
    assert(!r.empty);
    assert(r.front.isAnsiSequence);
    assert(r.front.slice == "\033[38;2;255;128;64;1;4;48;5;2m");

    r = "\033[mABC\033[m".ansiAsSections;
    assert(r.front.isAnsiSequence);
...
/++
 ...
 + ++/
@safe @nogc
AnsiSectionRange ansiAsSections(const(char)[] slice) nothrow pure
{
